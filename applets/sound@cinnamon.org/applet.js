...
class CinnamonSoundApplet extends Applet.TextIconApplet {
    ...
    _showFixedElements() {
        ...
        // 47 Industries System Sounds slider
        this._47soundItem = new PopupMenu.PopupBaseMenuItem({ activate: false });

        let _47soundBox = new St.BoxLayout({ vertical: false });

        // Speaker icon (clickable to mute/unmute)
        this._47soundIcon = new St.Icon({
        ...
        this._47soundIcon.connect('button-press-event', () => {
            GLib.spawn_command_line_async(GLib.get_home_dir() + "/.local/bin/47sound toggle");
            // Delay reading state to let the file update
        ...
        _47soundBox.add_actor(this._47soundIcon);

        // Label
        this._47soundLabel = new St.Label({ text: "47 Sounds: 100%", y_align: Clutter.ActorAlign.CENTER });
        this._47soundLabel.set_style("min-width: 110px; padding-right: 8px;");

        // Slider
        this._47soundSlider = new Slider.Slider(1.0);
        this._47soundSlider.connect("value-changed", (slider, value) => {
            let vol = Math.round(value * 100);
            this._47soundLabel.set_text("47 Sounds: " + vol + "%");
            GLib.spawn_command_line_async(GLib.get_home_dir() + "/.local/bin/47sound vol " + vol);
            if (vol > 0) {
                this._47soundIcon.icon_name = "xsi-audio-volume-high";
            } else {
                this._47soundIcon.icon_name = "xsi-audio-volume-muted";
            }
        });

        _47soundBox.add_actor(this._47soundLabel);
        _47soundBox.add(this._47soundSlider.actor, { expand: true });

        this._47soundItem.addActor(_47soundBox, { expand: true, span: -1 });
        this.menu.addMenuItem(this._47soundItem);
    ...
    _update47Sound() {
        try {
            let statePath = GLib.build_filenamev([GLib.get_home_dir(), ".config/47industries/sound-state"]);
            let [ok, contents] = GLib.file_get_contents(statePath);
            let text = contents.toString();
            let mutedMatch = text.match(/muted=(\w+)/);
            let volMatch = text.match(/volume=(\d+)/);
            let muted = mutedMatch ? (mutedMatch[1] === "true") : false;
            let vol = volMatch ? parseInt(volMatch[1]) : 100;

            if (muted) {
                this._47soundLabel.set_text("47 Sounds: Muted");
                this._47soundIcon.icon_name = "xsi-audio-volume-muted";
                this._47soundSlider.setValue(vol / 100);
            } else {
                this._47soundLabel.set_text("47 Sounds: " + vol + "%");
                this._47soundSlider.setValue(vol / 100);
                if (vol > 0)
            ...
            this._47soundLabel.set_text("47 Sounds: 100%");
            this._47soundSlider.setValue(1.0);
        }
    }
